...
export default {
    ...
    async mounted() {
        ...
        try {
            const pinfo = await import(`../assets/data/${pid}.js`)
            _t.problemInfo = pinfo.problemInfo
            _t.isLoading = false
        }
        catch (e) {
...
<template>
    ...
    <ul class="nav" :class="{
        ...
        <li class="nav-item">
            <a class="nav-link primary" :class="{ active: practiceMode == 1 }" href="javascript:;" @click="ruffleProblem()">
        ...
        <div class="row">
            <div class="col-md-9 col-sm-12" style="height: 100%;" :class="{
                ...
                <div class="height-100" :class="{ fade: fading }" style="transition: all .2s">
                    <div class="problem-show" v-if="setPrac">
                        <h4>设定练习题库</h4>
                        <div class="field">
                    ...
                    <div class="problem-show" v-else>
                        <div class="problem-content mb-4">
                            <span class="badge badge-primary mr-2">{{ types[nowProblems[nowId].type] }}</span>{{ nowId + 1
                            }}/{{
    nowProblems.length
}}.
                        ...
                        <div class="answer-area mb-4">
                            <template v-if="nowProblems[nowId].type === 3">
                                ...
                                <div v-if="problemState[nowId] === 3" class="form-group">
                                    <input class="form-control form-control-alternative input-success readonly" readonly
                                        :value="nowProblems[nowId].answer">
                                </div>
                            </template>
                            <template v-else>
